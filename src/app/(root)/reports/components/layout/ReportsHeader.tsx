...
function presetRange(
  ...
  const resolvedDateFrom =
    maxRange && toIsoDate(from) < maxRange.dateFrom
      ? maxRange.dateFrom
      : toIsoDate(from);
  const resolvedDateTo =
    maxRange && toIsoDate(anchor) > maxRange.dateTo
      ? maxRange.dateTo
      : toIsoDate(anchor);
...
function resolveActivePreset(
  ...
  const presetValues: QuickRangePreset[] = [
    "7d",
    "30d",
    "90d",
    "mtd",
...
export default function ReportsHeader({
  ...
  const [draftRange, setDraftRange] =
    useState<[Date | null, Date | null]>(rangeValue);
  const activeDateCountByIso = useMemo(
    () =>
      new Map(
        (payload.activeDates?.days ?? []).map((item) => [
          item.date,
          item.activeEntityCount,
        ]),
      ),
    [payload.activeDates],
  );
  ...
  const isSummaryRangeReport = isMaxRangeReport;
  ...
  const activePreset = useMemo(
    () =>
      isMaxRangeReport
        ? "max"
        : resolveActivePreset(
            payload.query.dateFrom,
            payload.query.dateTo,
            quickRangeMaxRange,
          ),
    [
      isMaxRangeReport,
  ...
  const rangeBadgeLabel = isMaxRangeReport
        ...
        >
          <div className={classes.headerMeta}>
            <Group gap="xs" align="center" wrap="wrap">
              ...
              {isDemo ? (
                <Badge color="orange" variant="outline" size="md">
                  Demo data
                </Badge>
              ) : null}
              <Badge
          ...
          {!isMaxRangeReport ? (
            <div className={classes.controlField}>
              ...
              <DatePickerInput
                ...
                minDate={
                  maxRange ? parseLocalDate(maxRange.dateFrom) : undefined
                }
                ...
                renderDay={(date) => {
                  ...
                  return (
                    <div className={classes.reportCalendarDay}>
                      ...
                      {activeCount > 0 ? (
                        <span className={classes.reportCalendarDayDot} />
                      ) : null}
                    </div>
                  );
                }}
          ...
          {payload.activeDates ? (
            <Group gap="xs" wrap="wrap" className={classes.activeDatesHint}>
              ...
              <Text size="xs" c="dimmed">
                {payload.activeDates.label}.{" "}
                {payload.activeDates.totalActiveDays.toLocaleString()} active
                day{payload.activeDates.totalActiveDays === 1 ? "" : "s"} across{" "}
                {activeDateSummary}.
              </Text>
            </Group>
          ) : null}
