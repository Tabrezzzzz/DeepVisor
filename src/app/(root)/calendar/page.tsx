...
function parseScope(value: string | string[] | undefined): BusinessIntelligencePlanningScope | undefined {
  ...
  if (raw === 'business' || raw === 'integration' || raw === 'selected_integrations') {
    return raw;
  }
...
export default async function CalendarPage({
  ...
  const workspace = await buildBusinessIntelligenceWorkspace(businessId, {
    scope: parseScope(params.scope),
    platformIntegrationId:
      typeof params.platform_integration_id === 'string' ? params.platform_integration_id : null,
  ...
  const intelligence = workspace.selectedAdAccountId
    ...
    : { signals: [], queueItems: [] };
  const queueTemplates = workspace.selectedAdAccountId
    ...
    : [];
  const campaignReviewOptions = workspace.selectedAdAccountId
    ? (
